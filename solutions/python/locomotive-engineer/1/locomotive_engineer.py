...
def fix_list_of_wagons(each_wagons_id:list[int], missing_wagons:list[int])->list[int]:
    ...
    whole_train = [locomotive, *missing_wagons, *rest, w1, w2]
    return whole_train
...
def extend_route_information(route:dict[str, str],
                             more_route_information:dict[str, str])->dict[str, str]:
...
def fix_wagon_depot(wagons_rows: list[list[str | int]])->list[list[str | int]]:
